...
- There is no `delete`. Removing an item means editing `TODO.md` by hand; `undone` is the only
  built-in way to walk a change back, and it flips the checkbox without stripping a `ref`.
...
- Headings are invisible to the numbering: `list`/`next`/`show`/`done` count only checklist
  lines, so adding a section above an item never renumbers it. `tests/todo-script.sh` pins that.
